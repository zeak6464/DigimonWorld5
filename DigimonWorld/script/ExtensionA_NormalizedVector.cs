...
namespace NormalizedVector
{
    public class NormalizedVectorClass : BakinObject
    {
...
/*
        ...
        */
        ...
        public float NormalizedVector3D_X(Vector3 a, Vector3 b)
        {
            ...
            return Vector3.Normalize(Vector3.Subtract(a, b)).X;
        }
        
        public float NormalizedVector3D_Y(Vector3 a, Vector3 b)
        {
            // [BakinFunction] を付与したメソッドはイベントパネル「C#プログラムの呼び出し」からコールできます。int/float/string の戻り値および引数を一つまで取ることができます。
            // One of the methods with [BakinFunction] can be called from the event panel "Calling C# Programs".  Up to one int/float/string return value and parameter can be used.

            return Vector3.Normalize(Vector3.Subtract(a, b)).Y;
        }

        
        public float NormalizedVector3D_Z(Vector3 a, Vector3 b)
        {
            // [BakinFunction] を付与したメソッドはイベントパネル「C#プログラムの呼び出し」からコールできます。int/float/string の戻り値および引数を一つまで取ることができます。
            // One of the methods with [BakinFunction] can be called from the event panel "Calling C# Programs".  Up to one int/float/string return value and parameter can be used.
            
            return Vector3.Normalize(Vector3.Subtract(a, b)).Z;
        }

        public float NormalizedVector2D_X(Vector3 a, Vector3 b)
        {
            // [BakinFunction] を付与したメソッドはイベントパネル「C#プログラムの呼び出し」からコールできます。int/float/string の戻り値および引数を一つまで取ることができます。
            // One of the methods with [BakinFunction] can be called from the event panel "Calling C# Programs".  Up to one int/float/string return value and parameter can be used.
            var a2D = new Vector3(a.X, 0, a.Z);
            var b2D = new Vector3(b.X, 0, b.Z);
            // var NVec = Vector3.Normalize(Vector3.Subtract(a2D, b2D));
            return Vector3.Normalize(Vector3.Subtract(a2D, b2D)).X;

        }

        public float NormalizedVector2D_Z(Vector3 a, Vector3 b)
        {
            // [BakinFunction] を付与したメソッドはイベントパネル「C#プログラムの呼び出し」からコールできます。int/float/string の戻り値および引数を一つまで取ることができます。
            // One of the methods with [BakinFunction] can be called from the event panel "Calling C# Programs".  Up to one int/float/string return value and parameter can be used.
            var a2D = new Vector3(a.X, 0, a.Z);
            var b2D = new Vector3(b.X, 0, b.Z);
            // var NVec = Vector3.Normalize(Vector3.Subtract(a2D, b2D));
            return Vector3.Normalize(Vector3.Subtract(a2D, b2D)).Z;
