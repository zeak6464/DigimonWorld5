...
namespace Yukar.Battle
{
    ...
    class BattleContentGetter : BattleContentGetterBase
    {
            ...
            GameContentCreator owner, RenderContent result, ref GameContentParser.StatusUpDown refStatusUpDown)
        {
            ...
            switch (content.type)
            {
                ...
                case GameContentParser.ContentType.ENEMYSTATUS_A:
                    {
                        ...
                        if (heros != null && heros.Count > index)
                        {
                            ...
                            {
                                return owner.GetGrowthFactor(hero, content.attrs[0]);
                            }

                            var level = Math.Min(hero.monsterGameData.level, Catalog.sInstance.getGameSettings().MaxLevel);
                            ...
                            hero.monster.levelUpExpList[level - 1] - hero.monsterGameData.exp,
                            hero.Attack,
                            hero.ElementAttack,
                            hero.Defense,
                            ...
                            hero.monsterGameData.maxLevel,
                            hero.monsterGameData.CurrentExp,
                        };

                            // 最大レベルのときは数値を返さない
                            // Does not return numbers when at maximum level
                            if (content.attrs[0] == 10 && hero.monsterGameData.IsMaxLevel)
                                return "-";
                ...
                case GameContentParser.ContentType.PARTYIMAGEICON_A:
                    {
                        ...
                        else
                        {
                            ...
                            if (heros.Count > partyIndex)
                            {
                                var hero = heros[partyIndex].player;
                                for (var i = 0; i < gameMain.data.party.PlayersInMenu.Count; ++i)
                                {
                                    if (hero == gameMain.data.party.PlayersInMenu[i])
                                    {
                                        owner.SetPartyIconThumbnail(result, gameMain.data, i);
                                        result.HasPartyElement = true;
                                        break;
                                    }
                                }

                                if (!result.HasPartyElement)
                ...
                case GameContentParser.ContentType.PARTYSTATUS_A_A:
                    {
                        ...
                        if (heros != null && partyIndex > -1 && heros.Count > partyIndex)
                        {
                            ...
                            if (content.growthFactor)
                            {
                                return owner.GetGrowthFactor(hero, content.attrs[1]);
                            }

                            var level = Math.Min(hero.player.level, Catalog.sInstance.getGameSettings().MaxLevel);
                            var status = new int[] {
                            hero.player.level,
                            hero.MaxHitPoint,
                            hero.battleStatusData.HitPoint,
                            ...
                            hero.Speed,
                            hero.player.exp,
                            hero.player.rom.levelUpExpList[level - 1] - hero.player.exp,
                            hero.Attack,
                            hero.ElementAttack,
                            hero.Defense,
                            Math.Min(100, hero.Dexterity),
                            ...
                            hero.player.maxLevel,
                            hero.player.CurrentExp,
                        };

                            // 最大レベルのときは数値を返さない
                            // Does not return numbers when at maximum level
                            if (content.attrs[0] == 10 && hero.player.IsMaxLevel)
                                return "-";
                            ...
                            if (status.Length > content.attrs[1])
                            {
                                if (gameContent.CharacterLevelUpData.Count > partyIndex)
                                {
                                    ...
                                    // 最大レベルのときは数値を返さない
                                    // Does not return numbers when at maximum level
                                    if (content.attrs[1] == 10 && characterLevelUpData.castLevelUpData.IsLevelMax)
                                        return "-";

                                    status[0] = characterLevelUpData.castLevelUpData.CurrentLevel;
                                    status[9] = characterLevelUpData.castLevelUpData.totalExp;
                                    status[10] = characterLevelUpData.castLevelUpData.CurrentNextLevelNeedExp;
                                    status[19] = characterLevelUpData.castLevelUpData.CurrentExp;
                                }
                ...
                case GameContentParser.ContentType.PARTYSTATUS_A:
                    {
                        ...
                        if (heros.Count > index)
                        {
                            ...
                            if (gameContent.CharacterLevelUpData.Count > index)
                            {
                                ...
                                // 最大レベルのときは数値を返さない
                                // Does not return numbers when at maximum level
                                if (content.attrs[0] == 10 && characterLevelUpData.castLevelUpData.IsLevelMax)
                                    return "-";
                ...
                case GameContentParser.ContentType.CURRENTPARTYIMAGE:
                    {
                        ...
                        {
                            if (!result.HasPartyElement)
                                owner.SetPartyImageThumbnail(result, hero.rom);
                            result.HasPartyElement = true;
                        }
                ...
                case GameContentParser.ContentType.CURRENTPARTYGRAPHIC:
                    {
                        ...
                        {
                            if (!result.HasPartyElement)
                                owner.SetPartyThumbnail(result, hero.rom);
                            result.HasPartyElement = true;
                        }
                ...
                case GameContentParser.ContentType.ITEMNAME:
                    {
						...
						{
                            return gameMain.catalog.getGameSettings().glossary.noItem;
                        }
                        else
                        {
                            return "";
                ...
                case GameContentParser.ContentType.ITEMNUM:
                    ...
                    #endregion
                case GameContentParser.ContentType.BATTLEEXT_A:
                    {
                        if (content.attrs[0] == 0)
                        {
                            var chr = battle.GetTurnOrdererdCharacter(index);
                            if (chr is BattlePlayerData)
                            {
                                owner.SetPartyIconThumbnail(result, ((BattlePlayerData)chr).player.rom);
                            }
                            else if (chr is BattleEnemyData)
                            {
                                owner.SetPartyIconThumbnail(result, ((BattleEnemyData)chr).monster);
                            }
                            return "";
                        }
                    }
        ...
        private static int[] CalclateHeroStatus(Common.GameData.Hero hero, Cast cast, int level, CharacterLevelUpData clud, Catalog catalog, AbstractRenderObject.GameContent gameContent)
        {
            ...
            var status = new int[] {
                level,
                hero.calcStatus(cast.hpParam, level, true) + hero.itemEffect.maxHitpoint,
                chr.player.HitPoint,
          ...
          SliderRenderer owner)
        {
            ...
            if (gameMain?.mapScene?.isBattle ?? false)
            {
                ...
                switch (content.type)
                {
                    case GameContentParser.ContentType.PARTYSP_A_A:
                        if ((content.attrs.Count > 1) && (heros.Count > index))
                        {
                            if (GetSliderValue(heros[index], content.attrs[1], gameMain.catalog, owner))
                            {
                                return true;
                            }
                        }
                        ...
                        return true;
                    case GameContentParser.ContentType.PARTYSP_A:
                        if ((content.attrs.Count > 0) && (owner.PageIndex > -1) && (heros.Count > owner.PageIndex))
                        {
                            if (GetSliderValue(heros[owner.PageIndex], content.attrs[0], gameMain.catalog, owner))
                            {
                                return true;
                            }
                        }
                    ...
                    case GameContentParser.ContentType.ENEMYSPPERCENT_A:
                        if ((content.attrs.Count > 0) && (menuIndex > -1) && (enemies.Count > menuIndex))
                        {
                            var enemy = enemies[menuIndex];
                            var no = content.attrs[0];

                            if (content.growthFactor)
                            {
                                var catalog = gameMain.catalog;

                                if (GetSliderValue(enemy, no, catalog, owner))
                                {
                                    var info = catalog.getGameSettings().GetCastStatusParamInfo(no);

                                    if (info.guId == gs.maxHPStatusID)
                                    {
                                        owner.ChangeDrawableParentContainer((enemy.imageAlpha > 0.5f) && (enemy.battleStatusData.consumptionStatusValue.GetStatus(info.guId) > 0));
                                    }
                            ...
                            {
                                // MAXが0だった場合、ゲージが空っぽで表示できるようにする
                                // If MAX is 0, the gauge will be displayed empty.
                                var hp = no == 0;
                                var value = hp ? enemy.battleStatusData.HitPoint : enemy.battleStatusData.MagicPoint;
                                var max = hp ? enemy.MaxHitPoint : enemy.MaxMagicPoint;

                                owner.SetVariable(
                                    owner.VariableText,
                                    value,
                                    ((value == max) && (max == 0)) ? 1 : max,
                                    0,
                                    true);

								if (hp)
								{
                                    owner.ChangeDrawableParentContainer(value > 0 && enemy.imageAlpha > 0.5f);
                                }
                        ...
                        return true;
                    case GameContentParser.ContentType.PARTYEXT_A_A:
                        if (heros.Count > index)
                        {
                            var hero = heros[index] as BattleSequenceManager.ExBattlePlayerData;
                            owner.SetVariable(
                                owner.VariableText,
                                (int)(Math.Min(255, hero.turnGauge * 256)),
                                255,
                                0,
                                true);
                            return true;
                        ...
                        break;
                    case GameContentParser.ContentType.ENEMYEXT_A_A:
                        if (enemies.Count > index)
                        {
                            var enm = enemies[index] as BattleSequenceManager.ExBattleEnemyData;
                            owner.SetVariable(
                                owner.VariableText,
                                (int)(Math.Min(255, enm.turnGauge * 256)),
                                255,
                                0,
                                true);
                            return true;
            ...
            else
            {
				switch (content.type)
				{
					...
					case GameContentParser.ContentType.PARTYHP_A:
                    case GameContentParser.ContentType.PARTYMP_A:
                    case GameContentParser.ContentType.ENEMYSP_A_A:
                    case GameContentParser.ContentType.ENEMYHP_A:
                    case GameContentParser.ContentType.ENEMYMP_A:
                        owner.SetVariable(
                            owner.VariableText,
                            0,
                            0,
		...
		private bool GetSliderValue(BattleCharacterBase inBattlePlayer, int inNo, Catalog inCatalog, SliderRenderer inOwner)
		{
            bool consumptionStatus;
            var info = inCatalog.getGameSettings().GetCastStatusParamInfo(inNo, out consumptionStatus);
            ...
            // MAXが0だった場合、ゲージが空っぽで表示できるようにする
            // If MAX is 0, the gauge will be displayed empty.
            var value = inBattlePlayer.battleStatusData.consumptionStatusValue.GetStatus(info.guId);
            var max = inBattlePlayer.battleStatusData.statusValue.GetStatus(info.guId);

            inOwner.SetVariable(
                inOwner.VariableText,
                value,
                ((value == max) && (max == 0)) ? 1 : max,
                0,
                true);
