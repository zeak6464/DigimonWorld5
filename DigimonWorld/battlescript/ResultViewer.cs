...
namespace Yukar.Battle
{
    ...
    public class ResultViewer
    {
        ...
        Resource.Icon.Ref newIcon = null;
        ...
        public void LoadResourceData(Catalog catalog)
        {
            ...
            newIcon = owner.catalog.getGameSettings().GraphicNewSprite;
            if (newIcon == null)
            {
                newIcon = new Resource.Icon.Ref(Resource.Texture.getDefaultCommonTexture(Resource.Texture.CommonTexture.NewIcon).guId, 0, 0);
            }
            if (newIcon != null)
            {
                newImageId = owner.catalog.getItemFromGuid(newIcon.guId) as Resource.Texture;
                ...
                if (resultWindowInfo.Value.HasFlag(ResultWindowInfo.Item))
                {
                    ...
                    foreach (var item in dropItemCountTable.Keys.Skip((itemPageCount - 1) * ItemPageDisplayCount).Take(ItemPageDisplayCount))
                    {
                        ...
                        if (isItemNewIconVisible && dropItemFirstGetTable[item])
                        {
                            ...
                            Graphics.DrawChipImage(newImageId, (int)pos.X, (int)pos.Y, newIcon.x, newIcon.y);
                        }
